...
Final production verification: 925bcf9 deployed Ready as dpl_2ynbesvNCmhbjZASmukJkow7BHYi. The same saved result now returns the corrected authored explanation on the next coach request; production-coach-fixed.txt records both the original failure and the corrected response. Signed-out access still redirects to authentication.
