...
import { askCoach, buildAnalystContext } from "../src/lib/ai";
...
test("gym research costs time without presenting invented records as historical facts", () => {
  ...
  assert.equal(
    opening.research.reduce((sum, item) => sum + item.hours, 0),
    7,
  );
  ...
  assert.match(
    researched.options.find((item) => item.id === "pilot")!.description,
    /1,000/,
  );
});
test("gym comparison uses 90 days of sales and bills for each plan", () => {
  ...
  assert.deepEqual(
    rows.map((row) => row.sales - row.costs),
    [-1500, -9500, 4500],
  );
});

test("gym alternative coaching preserves the full cost shortfall", async () => {
  const answer = await askCoach(getCase("case-031"), {
    caseId: "case-031",
    version: 2,
    researchIds: [],
    decision: { optionId: "pilot", reasoning: "", confidence: "medium" },
    messages: [
      { role: "user", text: "When would another choice work better?" },
    ],
  });
  assert.match(answer.answer, /9,500/);
  assert.match(answer.answer, /even if everyone pays/);
});
